...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "Game":
			resolverName, err := entityResolverNameForGame(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Game": %w`, err)
			}
			switch resolverName {
			...
			case "findGameByID":
				...
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findGameByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindGameByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Game": %w`, err)
				}
		...
		case "Player":
			resolverName, err := entityResolverNameForPlayer(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Player": %w`, err)
			}
			switch resolverName {
			...
			case "findPlayerByID":
				id0, err := ec.unmarshalNID2string(ctx, rep["id"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findPlayerByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindPlayerByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Player": %w`, err)
				}
...
func entityResolverNameForGame(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findGameByID", nil
	}
	return "", fmt.Errorf("%w for Game", ErrTypeNotFound)
}

func entityResolverNameForPlayer(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
			m   map[string]interface{}
			val interface{}
		...
		return "findPlayerByID", nil
	}
	return "", fmt.Errorf("%w for Player", ErrTypeNotFound)
}
